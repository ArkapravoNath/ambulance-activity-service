...
import com.gok.medicalpractitioners.config.KafkaProperties;
...
class AmbulanceactivityserviceKafkaResourceIT {
    ...
    void setup() {
        ...
        AmbulanceactivityserviceKafkaResource kafkaResource = new AmbulanceactivityserviceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ambulanceactivityservice-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/ambulanceactivityservice-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
